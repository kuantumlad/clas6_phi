...
int hadronBetaTool( int filestart = 1, int Nfiles = 1){
  ...
  std::vector<TH2F*> delta_beta_all_pr_sector;
  std::vector<TH2F*> delta_beta_all_pip_sector;
  std::vector<TH2F*> delta_beta_all_kp_sector;
  std::vector<TH2F*> delta_beta_all_pim_sector;
  std::vector<TH2F*> delta_beta_all_km_sector;
  ...
  for( int s = 0; s <= 6; s++ ){
    ...
    delta_beta_all_pr_sector.push_back( new TH2F(Form("h_delta_beta_pr_sector_%d",s),Form("h_delta_beta_pr_sector_%d",s), 250, 0.01, 5.0, 250, -0.5, 0.5));
    delta_beta_all_pip_sector.push_back( new TH2F(Form("h_delta_beta_pip_sector_%d",s),Form("h_delta_beta_pip_sector_%d",s), 250, 0.01, 5.0, 250, -0.5, 0.5));
    delta_beta_all_kp_sector.push_back( new TH2F(Form("h_delta_beta_kp_sector_%d",s),Form("h_delta_beta_kp_sector_%d",s), 250, 0.01, 5.0, 250, -0.5, 0.5));
    delta_beta_all_pim_sector.push_back( new TH2F(Form("h_delta_beta_pim_sector_%d",s),Form("h_delta_beta_pim_sector_%d",s), 250, 0.01, 5.0, 250, -0.5, 0.5));
    delta_beta_all_km_sector.push_back( new TH2F(Form("h_delta_beta_km_sector_%d",s),Form("h_delta_beta_km_sector_%d",s), 250, 0.01, 5.0, 250, -0.5, 0.5));
  }
  ...
  for(int i = 0; i < h22chain->GetEntries(); i++){    
    ...
    if( e_index[1] >= 0 ){
	  ...
	  int sc_paddle = sc_pd[k];
	  // exclude hadrons that hit a bad paddle. If this is not done beta vs p is distorted
	  if( !goodORbadSCpaddle( sc_sector, sc_paddle ) ) continue;
	  ...
	  if( q[k] > 0 && sc_sector >= 1 ){
	    //std::cout << " sector " << sc_sector << std::endl;
	    beta_all_pos_sector[sc_sector]->Fill(p[k], beta_measured);	
	  }
	  if( q[k] < 0 && sc_sector >= 1 && k != e_index[1] ){
	    //std::cout << " sector " << sc_sector << std::endl;
	    beta_all_neg_sector[sc_sector]->Fill(p[k], beta_measured);
	  }

	  if( q[k] < 0 && k != e_index[1] ){
	    //std::cout << " >> starting negative charge " << std::endl;
	    ...
	    std::vector<int> neg_pid_values;
	    neg_pid_values.push_back(-211);
	    neg_pid_values.push_back(-321);
	    
	    for( int c = 0; c < calc_beta_neg.size(); c++ ){
	      
	      float delta_t = fabs(ehtcorrMeasuredTime - (sc_r[k]/(calc_beta_neg[c]*speed_of_light) ));
	      
	      //std::cout << delta_t  << std::endl;
	      beta_map[neg_pid_values[c]] = delta_t;	  
	    ...
	    if( candidate_hadron == 0 ){
	      ...
	      beta_all_pim_sector[sc_sector]->Fill(p[k],beta_measured);
	    }
	    else if( candidate_hadron == 1 ){
	      //std::cout << " KM  " << std::endl;	
	      beta_all_km_sector[sc_sector]->Fill(p[k],beta_measured);
	    }
	      double db_pim = beta_measured - CalcBeta_pim;
	      delta_beta_all_pim_sector[sc_sector]->Fill(p[k],db_pim);

	      double db_km = beta_measured - CalcBeta_km;
	      delta_beta_all_km_sector[sc_sector]->Fill(p[k],db_km);
	...
	if( q[k] > 0 ){ 
	  ...
	  for( int c = 0; c < pid_values.size(); c++ ){
	  ...
	  //double db_pip = beta_measured - CalcBeta_pip;
	  //double db_pip = beta_measured - CalcBeta_pip;
	  //std::cout << " HADRON IS " << candidate_hadron << std::endl;
	    double db_pip = beta_measured - CalcBeta_pip;
	    delta_beta_all_pip_sector[sc_sector]->Fill(p[k],db_pip);
	    double db_kp = beta_measured - CalcBeta_kp;
	    delta_beta_all_kp_sector[sc_sector]->Fill(p[k],db_kp);
	    double db_pr = beta_measured - CalcBeta_pr;
	    delta_beta_all_pr_sector[sc_sector]->Fill(p[k],db_pr);
