...
int my_phi6(int iteration_number = 0,  int filestart = 1, int fileend = 2, int ExpOrSim = 1, bool do_momCorr_e = 0, bool do_momCorr_pions = 1, int cut_to_vary = 0, int cut_strictness = 0 ){ //int e_zvertex_strict = 0, int e_ECsampling_strict = 0, int e_ECoutVin_strict = 0, int e_ECgeometric_strict = 0, int e_CCthetaMatching_strict = 0, int e_R1fid_strict = 0, int e_R3fid_strict = 0, int e_CCphiMatching_strict = 0, int e_CCfiducial_strict = 0, int yCut_strict = 0, int pip_vvp_strict = 0, int pip_R1fid_strict = 0, int pip_MXcut_strict = 0, int pim_vvp_strict = 0, int pim_R1fid_strict = 0, int pim_MXcut_strict = 0)
  ...
  h22chain->SetBranchAddress("corr_hel", &corr_hel);
  ...
  TH1D *h_km_p = new TH1D("h_km_p","h_km_p", 200, 0.0, Beam_Energy);
  ...
  TH1D *h_km_p_pc = new TH1D("h_km_p_pc","h_km_p_pc", 200, 0.0, Beam_Energy);
  ...
  TH2D *h_deltime_p_km = new TH2D("h_deltime_p_km","h_deltime_p_km", 200, 0.0, 3.0, 200, -10.0, 10.0 );
  ...
  TH2D *h_pr_dcr1 = new TH2D("h_pr_dcr1","h_pr_dcr1", 500, -100.0, 100.0, 500, -100, 100 );
  TH2D *h_pr_dcr3 = new TH2D("h_pr_dcr3","h_pr_dcr3", 500, 50.0, 400.0, 500, -200.0, 200.0);

  TH2D *h_kp_dcr1 = new TH2D("h_kp_dcr1","h_kp_dcr1", 500, -100.0, 100.0, 500, -100, 100 );
  TH2D *h_kp_dcr3 = new TH2D("h_kp_dcr3","h_kp_dcr3", 500, 50.0, 400.0, 500, -200.0, 200.0);

  TH2D *h_km_dcr1 = new TH2D("h_km_dcr1","h_km_dcr1", 500, -100.0, 100.0, 500, -100, 100 );
  TH2D *h_km_dcr3 = new TH2D("h_km_dcr3","h_km_dcr3", 500, 50.0, 400.0, 500, -200.0, 200.0);
  ...
  for( int i = 0; i <= 6; i++ ){
    ...
    h_pr_sect_thetaphi.push_back( new TH2D(Form("h_pr_s%d_thetaphi",i), Form("h_pr_s%d_thetaphi",i), 200, 0.0, 65.0, 200, -180.0, 180.0));
    ...
    h_kp_sect_thetaphi.push_back( new TH2D(Form("h_kp_s%d_thetaphi",i), Form("h_kp_s%d_thetaphi",i), 200, 0.0, 65.0, 200, -180.0, 180.0) );

    h_km_sect_p.push_back( new TH1D(Form("h_km_s%d_p",i), Form("h_km_s%d_p",i), 200, 0.0, 4.0) );
    h_km_sect_p_pc.push_back( new TH1D(Form("h_km_s%d_p_pc",i), Form("h_km_s%d_p_pc",i), 200, 0.0, 4.0) );
    h_km_sect_betap.push_back( new TH2D(Form("h_km_s%d_betap",i), Form("h_km_s%d_betap",i), 200, 0.0, 4.0, 200, 0.0, 1.1) );
    h_km_sect_betap_pc.push_back( new TH2D(Form("h_km_s%d_betap_pc",i), Form("h_km_s%d_betap_pc",i), 200, 0.0, 4.0, 200, 0.0, 1.1) );
    h_km_sect_thetaphi.push_back( new TH2D(Form("h_km_s%d_thetaphi",i), Form("h_km_s%d_thetaphi",i), 200, 0.0, 65.0, 200, -180.0, 180.0) );
  ...
  // %%%%%%%%%%% pass rate for electrons %%%%%%%%%%%
  for( int i = 0; i < 9; i++ ){
    std::cout << " PASS RATE FOR CUT LVL " << i << ": " << el_pass_rate[i] << std::endl;
  }
