...
std::map<int,std::vector<double> > hadronMLEID(int hypothesis, int gpart, int e_index[], Int_t q[], Float_t p[], UChar_t sc_sect[], UChar_t dc_sect[], Float_t sc_t[], Float_t sc_r[], UChar_t sc_pd[], int pip_vvp_strict, int pip_R1fid_strict, int pip_MXcut_strict, int ExpOrSim, Float_t ec_ei[], UChar_t ec_sect[], UChar_t cc_sect[], UShort_t nphe[], Float_t ec_eo[], Float_t cx[], Float_t cy[], Float_t cz[], Float_t b[], Float_t tl1_x[], Float_t tl1_y[],  TLorentzVector V4_H[], int currentrunno, int pim_vvp_strict, int pim_R1fid_strict, int pim_MXcut_strict, std::map<int, std::vector<double> > pr_mean_fit, std::map<int, std::vector<double> > pip_mean_fit, std::map<int, std::vector<double> > kp_mean_fit, std::map<int, std::vector<double> > prot_sigma_fit, std::map<int, std::vector<double> > pip_sigma_fit, std::map<int, std::vector<double> > kp_sigma_fit, double pr_conf, double pip_conf, double kp_conf, double pr_anticonf, double pip_anticonf, double kp_anticonf )
{
  ...
  for(int k = 0; k < gpart; k++) // loop over particles
	...
	if( R1fid_pass ){
	  ...
	  double deltab_pip = beta_measured - CalcBeta_pip;
	  double mean_pip =  pip_mean_c[sc_sector-1]*pow(mom,2) + pip_mean_b[sc_sector-1]*mom + pip_mean_a[sc_sector-1];
	  double sigma_pip =  pip_sigma_c[sc_sector-1]*pow(mom,2) + pip_sigma_b[sc_sector-1]*mom + pip_sigma_a[sc_sector-1];
	  double prob_pip = (1/(sigma_pip*sqrt(2*3.14159))) * exp(-0.5 * pow((deltab_pip + mean_pip)/sigma_pip, 2));
	  double conf_pip = (1.0 - TMath::Erf(fabs(deltab_pip + mean_pip)/sigma_pip/sqrt(2.0))); 
	  ...
	  double deltab_kp = beta_measured - CalcBeta_kp;		  
	  double mean_Kp =  Kp_mean_c[sc_sector-1]*pow(mom,2) + Kp_mean_b[sc_sector-1]*mom + Kp_mean_a[sc_sector-1];
	  double sigma_Kp =  Kp_sigma_c[sc_sector-1]*pow(mom,2) + Kp_sigma_b[sc_sector-1]*mom + Kp_sigma_a[sc_sector-1];
	  double prob_Kp = (1/(sigma_Kp*sqrt(2*3.14159))) * exp(-0.5 * pow((deltab_kp + mean_Kp)/sigma_Kp, 2));
	  double conf_Kp = (1.0 - TMath::Erf(fabs(deltab_kp + mean_Kp)/sigma_Kp/sqrt(2.0)));        
	  ...
	  if(prob_pip > prob_prot && prob_pip  > prob_Kp  && hypothesis == 211 ){
	  ...
	  if(prob_Kp > prob_prot && prob_Kp > prob_pip && hypothesis == 321 ){
